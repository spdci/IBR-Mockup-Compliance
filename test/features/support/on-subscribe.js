...
Given(/^SP has previously sent a subscribe request to IBR$/, function () {
  specSubscribe = spec(); // Initialize the specSearch object
});

When(/^IBR completes processing and calls SP on-subscribe callback$/, async function () {
  try {
...
Then(/^SP should receive the on-subscribe response from IBR$/, async function () {
