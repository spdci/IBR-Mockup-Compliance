...
Given(/^SP has previously sent an unsubscribe request to IBR$/, function () {
  specUnsubscribe = spec(); // Initialize the specSearch object
});

When(/^IBR completes processing and calls SP on-unsubscribe callback$/, async function () {
  try {
...
Then(/^SP should receive the on-unsubscribe response from IBR$/, async function () {
